...
import { GetRelationListResponseRequest, getSearchBoardListRequest } from 'apis';
...
export default function Search() {
  ...
  //  state:  searchWord path vaeiable 상태  //
  const { searchWord } = useParams();
  // state:  페이지 네이션 관련 상태  //
  const { currentPage, setCurrentPage, currentSection, 
  setCurrentSection, viewList, viewPageList, totalSection,
  ...
  useEffect(() => {
    ...
    GetRelationListResponseRequest(searchWord).then(getRelationListResponse);
  }, [searchWord]);
  ...
  return (
    <div id='search-wrapper'>
      <div className='search-container'>
        ...
        {count !== 0 && <Pagination currentPage={currentPage} currentSection={currentSection} setCurrentPage={setCurrentPage} setCurrentSection={setCurrentSection} viewPageList={viewPageList} totalSection={totalSection}/>}
        </div>
      </div>
    </div>
  )
